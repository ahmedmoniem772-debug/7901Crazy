    ...
    public class RebornInfomations : List<Tuple<byte, uint, uint, RebornInfomations.Action, List<ushort>>>
    {
        ...
        public unsafe void Reborn(Role.Player player, uint RebornClass, ServerSockets.Packet stream)
        {
            ...
            if (RebornClass % 10 == 1 || RebornClass == 13002 || RebornClass == 14002)
            {
                ...
                if (client.MyArchives.AirPowers.Count == 0)
                {
                    var Power = new Role.Instance.Archives.items();
                    Power.Level = 1;
                    Power.Exp = 0;
                    Power.Type = (Role.Instance.Archives.AirTypeID)701;
                    client.MyArchives.AirPowers.Add((Role.Instance.Archives.AirTypeID)701, Power);

                    var Powers = new Role.Instance.Archives.items();
                    Powers.Level = 1;
                    Powers.Exp = 0;
                    Powers.Type = (Role.Instance.Archives.AirTypeID)702;
                    client.MyArchives.AirPowers.Add((Role.Instance.Archives.AirTypeID)702, Powers);

                    var Powerss = new Role.Instance.Archives.items();
                    Powerss.Level = 1;
                    Powerss.Exp = 0;
                    Powerss.Type = (Role.Instance.Archives.AirTypeID)703;
                    client.MyArchives.AirPowers.Add((Role.Instance.Archives.AirTypeID)703, Powerss);
                }
                client.MyArchives.Close();
                client.MyArchives.Loading();
                if (!Database.AtributesStatus.IsTaoist(client.Player.Class))
                {
                    var WaterAegis = client.MyArchives.Items.Values.Where(p => p.ItemID >= Role.Instance.Archives.TypeID.Vicissitude && p.ItemID <= Role.Instance.Archives.TypeID.Birthdeath).ToArray();
                    if (WaterAegis.Length > 0)
                    {
                        if (!client.MySpells.ClientSpells.ContainsKey((ushort)Role.Flags.SpellID.WaterAegisRebirth))
                            client.MySpells.Add(stream, (ushort)Role.Flags.SpellID.WaterAegisRebirth);
                    }
                    else
                    {
                        client.MySpells.Remove((ushort)Role.Flags.SpellID.WaterAegisRebirth, stream);
                    }
