    ...
    public class Disdain
    {
        ...
        public static int UserAttackUser(Role.Player attacker, Role.Player target, int damage)
        {
            ...
            if (target.Owner.Rune.IsEquipped("VioletShield", ref VioletShieldL))
            {
                ...
                if (VirusX.Game.MsgServer.AttackHandler.Calculate.Base.Rate(chance))
                {
                    work = true;
                }
            }
            #endregion
            var disdain = LookupDisdain(attacker.BattlePower, ((attacker.ContainFlag(Game.MsgServer.MsgUpdate.Flags.DeathSigh) && !work) && target.BattlePower > attacker.BattlePower ? attacker.BattlePower : target.BattlePower));
