    ...
    public class ThunderRampage
    {
        public unsafe static void Execute(Client.GameClient user, InteractQuery Attack, ServerSockets.Packet stream, Dictionary<ushort, Database.MagicType.Magic> DBSpells)
        {
            ...
            if (CheckAttack.CanUseSpell.Verified(Attack, user, DBSpells, out ClientSpell, out DBSpell))
            {
                ...
                {
                    case (ushort)Role.Flags.SpellID.ThunderRampage:
                        {
                            ...
                            if (user.Player.ContainFlag(MsgUpdate.Flags.XPList))
                            {
                                ...
                                user.Player.OpenXpSkill(MsgUpdate.Flags.ThunderRampage, 40);
                            }
                            else if (user.Player.ContainFlag(MsgUpdate.Flags.ThunderRampage))
                            {
                                uint Experience = 0;
                                foreach (Role.IMapObj target in user.Player.View.Roles(Role.MapObjectType.Monster))
                                {
                                    MsgMonster.MonsterRole attacked = target as MsgMonster.MonsterRole;
                                    if (Calculate.Base.GetDistance(Attack.X, Attack.Y, attacked.X, attacked.Y) <= 11)
                                    {
