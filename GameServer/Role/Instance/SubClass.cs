...
namespace VirusX.Role.Instance
{
    public class SubClass
    {
            ...
            foreach (var item in src.Values)
            {
                switch (item.ID)
                {
                    case Database.DBLevExp.Sort.Apothecary:
                        {
                            client.Status.Detoxication += Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0];
                            break;
                        }
                    case Database.DBLevExp.Sort.ChiMaster:
                        {
                            client.Status.Immunity += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0] * 100);
                            break;
                        }
                    case Database.DBLevExp.Sort.MartialArtist:
                        {
                            client.Status.CriticalStrike += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0] * 100);
                            break;
                        }
                    case Database.DBLevExp.Sort.Performer:
                        {
                            client.Status.MagicAttack += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0]);
                            client.Status.MaxAttack += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[1]);
                            client.Status.MinAttack += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[1]);
                            break;
                        }
                    case Database.DBLevExp.Sort.Sage:
                        {
                            client.Status.Penetration += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0] * 100);
                            break;
                        }
                    case Database.DBLevExp.Sort.Warlock:
                        {
                            client.Status.SkillCStrike += (uint)(Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0] * 100);
                            break;
                        }
                    case Database.DBLevExp.Sort.Wrangler:
                        {
                            client.Status.MaxHitpoints += Pool.SubClassInfo.GetDamage(item.ID, item.Phrase)[0];
                            break;
                        }
                }
            }
        ...
        {
            return user.Player.SubClass.SendLearn(user, Game.MsgServer.MsgSubClass.SubClases.Create2(type), stream);
        }
        ...
        public void UpdateStundyPoints(Client.GameClient client, ushort amount, ServerSockets.Packet stream)
        {
            client.Player.SubClass.StudyPoints += amount;
            UpdateStudyPacket(client, stream);
        }
        ...
        internal unsafe void LearnFixedSubClass(Database.DBLevExp.Sort sort, Client.GameClient client, ServerSockets.Packet stream)
        {
