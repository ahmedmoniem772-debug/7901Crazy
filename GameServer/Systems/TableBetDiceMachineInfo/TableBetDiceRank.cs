...
{
    public class TableBetDiceRank
    {
        public enum Type : uint
        {
           JiangHuFameRanking,//96
           JiangHuFameRankingPrevious,//97
            sayed,
            JiangHuFameRankingTotal,//99
            Count,
        }
        public class Entry
        {
            public TableBetDiceRank.Type Type;
            public uint UID;
            public string Name;
            public uint Class;
            public byte Level;
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(TableBetDiceRank.Entry BestOf)
            {
        ...
        public class Rank
        {
            public TableBetDiceRank.Type _Type;
            public int MaxItems;
            public Dictionary<uint, TableBetDiceRank.Entry> RankingList;
            ...
            public void UpdateItem(TableBetDiceRank.Entry item)
            {
                lock (SynRoot)
                {
        ...
        {
            TableBetDiceRank.Create();
            using (Read read = new Read("TableBetDiceRank.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    entry.Type = (TableBetDiceRank.Type)readLine.Read((byte)0);
                    ...
                    TableBetDiceRank.Ranks[entry.Type].UpdateItem(entry);
                }
            }
        }
        ...
        {
            using (Write write = new Write("TableBetDiceRank.txt"))
            {
