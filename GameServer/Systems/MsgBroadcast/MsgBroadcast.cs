...
namespace VirusX.Game.MsgServer
{
    public unsafe static partial class MsgBuilder
    {
        ...
        {
            Action = (MsgBroadcast.BroadTyp)stream.ReadUInt32();//4
            dwParam = stream.ReadUInt32();//8
            stream.ReadUInt32();//12
            Strings = stream.ReadStringList();//16
        ...
        public static unsafe ServerSockets.Packet BroadcastCreate(this ServerSockets.Packet stream, MsgBroadcast.BroadTyp Action, uint dwParam, string[] Strings)
        {
            ...
            stream.Finalize(GamePackets.MsgPigeon);
    ...
    public unsafe struct MsgBroadcast
    {
        ...
        [PacketAttribute(GamePackets.MsgPigeon)]
                    ...
                    case BroadTyp.BroadcastMessage:
                        {
                            ...
                            if (user.Player.ConquerPoints >= 5)
                            {
                                ...
                                if (user.Player.IsStillBanned)
                                {
                                    ...
                                    {
                                        user.SendSysMesage("Sorry, you still banned from chatting Permenatly.", VirusX.Game.MsgServer.MsgMessage.ChatMode.System, VirusX.Game.MsgServer.MsgMessage.MsgColor.white);
                                    }
                                    else
                                    {
                                        user.SendSysMesage("Sorry, you still banned from chatting till " + user.Player.BannedChatStamp.ToString(), VirusX.Game.MsgServer.MsgMessage.ChatMode.System, VirusX.Game.MsgServer.MsgMessage.MsgColor.white);
                                    }
                                    return;
                                }
                                if (user.Player.Map == 5040)
                                {
                                    user.SendSysMesage("Sorry, you still banned from chatting Permenatly. using BotJail, HurdLuck", VirusX.Game.MsgServer.MsgMessage.ChatMode.System, VirusX.Game.MsgServer.MsgMessage.MsgColor.white);
                                    return;
                                }
                                ...
                                if (DateTime.Now > user.Player.LastBroadCast.AddSeconds(65))
                                {
                                    if (MsgTournaments.MsgBroadcast.Broadcasts.Count == 0)
                                    {
                                        if (MsgTournaments.MsgBroadcast.CurrentBroadcast.EntityID == 1)
                                        {
                                            ...
                                            ServerLogs.AddChatLog(null, user, broadcast.Message);
                                            break;
                                        }
                                    }
                                }
