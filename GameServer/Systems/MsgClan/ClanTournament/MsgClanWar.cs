    ...
    public class MsgClanWar
    {
        ...
        public class CityWar
        {
...
#else
                    MsgSchedules.SendInvitation("" + Type.ToString() + "City Clan War", "Conquer Points", RequestX, RequestY, RequestMap, 0, 60);
#endif
                }
            }
            ...
            public void CompleteWar()
            {
                if (Proces == ProcesType.Alive)
                {
                    ...
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        Server.SendGlobalPacket(new MsgServer.MsgMessage(Winner.Name + "has claimed the prize for domination of " + this.Type.ToString() + "CityArena" + "."
            ...
            public void AddNpc(ushort x, ushort y)
            {
                ...
                Pole.Mesh = (Role.SobNpc.StaticMesh)31220;
                    ...
                    if (user.Player.DynamicID == DinamycID && user.Player.Map == Map.ID)
                    {
                        if (user.Player.ClanUID == Winner.ClainID)
                        {
                            if (user.Player.MyClan != null && user.Player.MyClanMember != null)
                            {
                                ...
                                {
                                    user.Player.ConquerPoints -= (long)NewSystem.PrizeAllTops.ClanWar;
#if Arabic
                                    user.CreateBoxDialog("You received " + ClanWarScrore.ConquerPointsReward + " ConquerPoints.");
#else
                                    user.CreateBoxDialog("You received " + NewSystem.PrizeAllTops.ClanWar + " ConquerPoints.");
#endif
                                    using (var rec = new ServerSockets.RecycledPacket())
                                    {
                                        var stream = rec.GetStream();
                                ...
                                {
                                    user.Player.ConquerPoints -= (long)NewSystem.PrizeAllTops.ClanWar / 4;
#if Arabic
                                    user.CreateBoxDialog("You received " + ClanWarScrore.ConquerPointsReward / 2 + " ConquerPoints.");
#else
                                    user.CreateBoxDialog("You received " + NewSystem.PrizeAllTops.ClanWar / 4 + " ConquerPoints.");
#endif
                                }
                            }
                        }
        ...
        public MsgClanWar()
        {
            try
            {
                ...
                WarCitys.Add(CityType.Twin, new CityWar(CityType.Twin, Pool.ServerMaps[1505], 165, 219, 1002, 327, 138));
                WarCitys.Add(CityType.Phoenix, new CityWar(CityType.Phoenix, Pool.ServerMaps[1509], 82, 119, 1011, 210, 260));
                WarCitys.Add(CityType.Ape, new CityWar(CityType.Ape, Pool.ServerMaps[1506], 108, 124, 1020, 568, 583));
                WarCitys.Add(CityType.Desert, new CityWar(CityType.Desert, Pool.ServerMaps[1508], 125, 143, 1000, 496, 673));//Done
                WarCitys.Add(CityType.Bird, new CityWar(CityType.Bird, Pool.ServerMaps[1507], 95, 113, 1015, 707, 571));
                if (Pool.Constants.FreePkMap.Contains(1505) == false)
                    Pool.Constants.FreePkMap.Add(1505);
                if (Pool.Constants.FreePkMap.Contains(1509) == false)
                    Pool.Constants.FreePkMap.Add(1509);
                if (Pool.Constants.FreePkMap.Contains(1506) == false)
                    Pool.Constants.FreePkMap.Add(1506);
                if (Pool.Constants.FreePkMap.Contains(1508) == false)
                    Pool.Constants.FreePkMap.Add(1508);
                if (Pool.Constants.FreePkMap.Contains(1507) == false)
                    Pool.Constants.FreePkMap.Add(1507);
