...
namespace VirusX.Game.MsgServer
{
    ...
    {
        public static unsafe ServerSockets.Packet ArenaRankingCreate(this ServerSockets.Packet stream, MsgArenaRanking.RankTyp rank, ushort page, uint Length, uint Count, uint ArenaID)
        {
            stream.InitWriter();
        ...
        public static unsafe ServerSockets.Packet AddItemArenaRanking(this ServerSockets.Packet stream, ushort Rank, string Name, ushort UnKnow, uint Points,
             uint Class, uint level, uint unknow2, uint Mesh)
        {
            stream.Write(Rank);
        ...
        {
            stream.Finalize(GamePackets.MsgQualifyingRank);
            return stream;
        }
    ...
    public unsafe struct MsgArenaRanking
    {
        ...
        [PacketAttribute(GamePackets.MsgQualifyingRank)]
                ...
                case RankTyp.Today:
                    {
                        ...
                        stream.ArenaRankingCreate(RankTyp.History, PageNumber, (uint)info.Length, (uint)count, 1U);
                            ...
                            if (info.Length > offset + x)
                            {
                                ...
                                if (element.Info.TodayRank < 1000)
                                {
                                    stream.AddItemArenaRanking((ushort)(offset + x + 1), element.Name, 0, element.Info.ArenaPoints
                                        , element.Class, element.Level, 1, element.Mesh);
                                }
                            }
                        }
                        ...
                        for (int x = 0; x < count; x++)
                        {
                            if (info.Length > offset + x)
                            {
                                ...
                                if (element.Info.TodayRank < 1000)
                                {
                                  ...
                                  , element.Class, element.Level, 33942209, element.Mesh);
                                }
                            }
                        }
                        user.Send(stream.ArenaRankingFinalize());
