...
namespace VirusX.Database
{
    public class WarriorRank
    {
        ...
        public class Entry
        {
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(WarriorRank.Entry BestOf)
            {
        ...
        {
            WarriorRank.Create();
            using (Read read = new Read("WarriorRank.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    item.UID = readLine.Read((uint)0);
                    item.Name = readLine.Read("");
                    item.Type = (WarriorRank.Type)readLine.Read((byte)0);
                    item.TotalPoints = readLine.Read((uint)0);
                    item.Class = readLine.Read((uint)0);
                    item.Level = readLine.Read((byte)0);
                    item.Mesh = readLine.Read((byte)0);
                    item.HairStyle = readLine.Read((uint)0);
                    item.Head = readLine.Read((uint)0);
                    item.Garment = readLine.Read((uint)0);
                    item.LeftWeapon = readLine.Read((uint)0);
                    item.LefttWeaponAccessory = readLine.Read((uint)0);
                    item.RightWeapon = readLine.Read(0U);
                    item.RightWeaponAccessory = readLine.Read((uint)0);
                    item.MountArmor = readLine.Read((uint)0);
                    item.Armor = readLine.Read((uint)0);
                    item.Wing = readLine.Read((uint)0);
                    item.WingPlus = readLine.Read((uint)0);
                    item.Title = readLine.Read((uint)0);
                    item.Flag = readLine.Read((uint)0);
                    item.GuildName = readLine.Read("");
                    item.WeaponWar = readLine.Read((uint)0);
                    WarriorRank.Ranks[item.Type].UpdateItem(item);
                }
            }
        }
        ...
        {
            using (Write write = new Write("WarriorRank.txt"))
            {
