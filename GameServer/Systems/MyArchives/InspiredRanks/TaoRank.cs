...
namespace ConquerOnline.Database
{
    public class TaoRank
    {
        ...
        public class Entry
        {
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(TaoRank.Entry BestOf)
            {
        ...
        public class Rank
        {
            public TaoRank.Type _Type;
            public int MaxItems;
            public Dictionary<uint, TaoRank.Entry> RankingList;
            ...
            public void UpdateItem(TaoRank.Entry item)
            {
                lock (SynRoot)
                {
        ...
        {
            TaoRank.Create();
            using (Read read = new Read("TaoRank.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    entry.Type = (TaoRank.Type)readLine.Read((byte)0);
                    ...
                    TaoRank.Ranks[entry.Type].UpdateItem(entry);
                }
            }
        }
        ...
        {
            using (Write write = new Write("TaoRank.txt"))
            {
