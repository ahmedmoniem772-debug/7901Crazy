...
namespace VirusX.Database
{
    public class WaterRank
    {
        ...
        public class Entry
        {
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(WaterRank.Entry BestOf)
            {
        ...
        public class Rank
        {
            public WaterRank.Type _Type;
            public int MaxItems;
            public Dictionary<uint, WaterRank.Entry> RankingList;
            ...
            public void UpdateItem(WaterRank.Entry item)
            {
                lock (SynRoot)
                {
        ...
        {
            WaterRank.Create();
            using (Read read = new Read("WaterRank.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    entry.Type = (WaterRank.Type)readLine.Read((byte)0);
                    ...
                    WaterRank.Ranks[entry.Type].UpdateItem(entry);
                }
            }
        }
        ...
        {
            using (Write write = new Write("WaterRank.txt"))
            {
