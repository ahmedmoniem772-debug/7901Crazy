...
namespace VirusX.Database
{
    public class FireRank
    {
        ...
        public class Entry
        {
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(FireRank.Entry BestOf)
            {
        ...
        public class Rank
        {
            public FireRank.Type _Type;
            public int MaxItems;
            public Dictionary<uint, FireRank.Entry> RankingList;
            ...
            public void UpdateItem(FireRank.Entry item)
            {
                lock (SynRoot)
                {
        ...
        {
            FireRank.Create();
            using (Read read = new Read("FireRank.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    entry.Type = (FireRank.Type)readLine.Read((byte)0);
                    ...
                    FireRank.Ranks[entry.Type].UpdateItem(entry);
                }
            }
        }
        ...
        {
            using (Write write = new Write("FireRank.txt"))
            {
