...
namespace VirusX.Database
{
    public class MonkRanks
    {
        ...
        public class Entry
        {
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(MonkRanks.Entry BestOf)
            {
        ...
        public class Rank
        {
            public MonkRanks.Type _Type;
            public int MaxItems;
            public Dictionary<uint, MonkRanks.Entry> RankingList;
            ...
            public void UpdateItem(MonkRanks.Entry item)
            {
                lock (SynRoot)
                {
        ...
        {
            MonkRanks.Create();
            using (Read read = new Read("MonkRanks.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    entry.Type = (MonkRanks.Type)readLine.Read((byte)0);
                    ...
                    MonkRanks.Ranks[entry.Type].UpdateItem(entry);
                }
            }
        }
        ...
        {
            using (Write write = new Write("MonkRanks.txt"))
            {
