...
namespace VirusX.Database
{
    public class AstredgeRank
    {
        ...
        public class Entry
        {
            ...
            public static implicit operator Game.MsgServer.MsgRankMemberShow.MsgRankMemberShowProto.Entry(AstredgeRank.Entry BestOf)
            {
        ...
        public class Rank
        {
            public AstredgeRank.Type _Type;
            public int MaxItems;
            public Dictionary<uint, AstredgeRank.Entry> RankingList;
            ...
            public void UpdateItem(AstredgeRank.Entry item)
            {
                lock (SynRoot)
                {
        ...
        {
            AstredgeRank.Create();
            using (Read read = new Read("AstredgeRank.txt"))
            {
                if (!read.Reader())
                    return;
                ...
                for (int index = 0; index < count; ++index)
                {
                    ...
                    entry.Type = (AstredgeRank.Type)readLine.Read((byte)0);
                    ...
                    AstredgeRank.Ranks[entry.Type].UpdateItem(entry);
                }
            }
        }
        ...
        {
            using (Write write = new Write("AstredgeRank.txt"))
            {
