   ...
   public class StaticConnexion
    {
       ...
       public static System.ThreadGroup.ThreadItem Thread = null;
       ...
       public static void Create()
       {
           if (Program.ServerConfig.IsInterServer == false)
           {
               ...
               if (Thread == null)
               {
                   Thread = new System.ThreadGroup.ThreadItem(10000, "InterServer", CheckConnection);
                   Thread.Open();
               }
           }
       }
       ...
       public static void ProcesReceive(ServerSockets.SecuritySocket obj, ServerSockets.Packet stream)
       {
           ...
           try
           {
               switch (PacketID)
               {
                   ...
                   case VirusX.Game.GamePackets.MsgTalk:
                       {
                           ...
                           if (mes.ChatType == MsgMessage.ChatMode.BroadcastMessage)
                           {
                               if (mes.__Message == "[Cross Elite PK Tournament] begins at 20:00. Get yourself prepared for it!")
                               {
                                   ...
                                   VirusX.Game.MsgTournaments.MsgSchedules.SendInvitation("Cross Elite PK Tournament", "[Special Accesory/Boots,Cps and more rewards]", 296, 163, 1002, 0, 60);
                                   break;
                               }
                               if (mes.__Message == "[Cross Capture The Flag] begins at 21:00. Get yourself prepared for it!")
                               {
                                   Core.IsCrossFlagOpen = true;
                                   Core.JoinCrossFlagStamp = DateTime.Now.AddHours(1);
                                   VirusX.Game.MsgTournaments.MsgSchedules.SendInvitation("Cross Capture The Flag ", "[Cps and more rewards]", 327, 325, 1002, 0, 60);
                                   break;
                               }
                               if (mes.__Message == "[Cross Guild War] begins at 21:00. Get yourself prepared for it!")
                               {
                                   Core.IsGWFlagOpen = true;
                                   Core.JoinGWFlagStamp = DateTime.Now.AddHours(1);
                                   VirusX.Game.MsgTournaments.MsgSchedules.SendInvitation("Cross Guild War", "[Cps and more rewards]", 327, 325, 1002, 0, 60);
                                   break;
                               }
                           }
