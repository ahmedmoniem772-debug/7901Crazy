...
namespace VirusX.Game.MsgServer
{
    ...
    {
        public static unsafe ServerSockets.Packet CreateProcessGoal(this ServerSockets.Packet stream, MsgProcessGoalSchedule obj)
        {
            stream.InitWriter();
            stream.ProtoBufferSerialize(obj);
            stream.Finalize(GamePackets.MsgProcessGoalSchedule);

            return stream;
        }
        public static unsafe void GetProcessGoal(this ServerSockets.Packet stream, out MsgProcessGoalSchedule pQuery)
        {
            pQuery = new MsgProcessGoalSchedule();
            pQuery = stream.ProtoBufferDeserialize<MsgProcessGoalSchedule>(pQuery);
        }
    }
    [ProtoContract]
    public class MsgProcessGoalSchedule
    {
        ...
        [PacketAttribute(GamePackets.MsgProcessGoalSchedule)]
        private static void Handler(Client.GameClient user, ServerSockets.Packet stream)
        {
            MsgProcessGoalSchedule Info = null;
            stream.GetProcessGoal(out Info);
            user.Send(stream.CreateProcessGoal(Info));
        }
    }
    ...
    public static unsafe partial class MsgBuilder
    {
        ...
        public static unsafe ServerSockets.Packet HeroRewardCreate(this ServerSockets.Packet stream, MsgHeroReward.ActionID mode, uint Prize)
        {
            ...
            stream.Finalize(GamePackets.MsgProcessGoalTaskOpt);
        ...
        public static unsafe ServerSockets.Packet HeroRewardlistCreate(this ServerSockets.Packet stream, uint dwParam, byte AllDone, Role.Instance.HeroRewards.Item[] array)
        {
            ...
            stream.Write(AllDone);//8
            if (array != null)
                stream.Write((ushort)array.Length);//5
            else
                stream.Write((ushort)0);//5

            if (array != null)
            {
                for (int x = 0; x < array.Length; x++)
                {
                    stream.Write(array[x].UID);//7
                    stream.Write(array[x].UnKnow);//11
                    stream.Write((byte)0);//15
                    stream.Write(array[x].Claim);//16
                }
            }

            stream.Finalize(GamePackets.MsgProcessGoalTask);
    ...
    public unsafe struct MsgHeroReward
    {
        ...
        [PacketAttribute(GamePackets.MsgProcessGoalTaskOpt)]
        private static void Handler(Client.GameClient user, ServerSockets.Packet stream)
        {
         ...
         switch (mode)
            {
                case ActionID.Show:
                    {
                        ...
                        var DBInfo = Pool.TableHeroRewards[Prize];
                            ...
                            case 4:
                                {
                                    if (DBInfo.CheckOpen(user.Player.Level, user.Player.Reborn))
                                    {
                                        ...
                                        //404 Go to Ape City and take the challenge of `Dis City`, once.
                            ...
                            case 5:
                                {
                                    if (DBInfo.CheckOpen(user.Player.Level, user.Player.Reborn))
                                    {
                                        ...
                                        //505 Join the Champion`s Arena, once.
                                        //506 Win 20 fights in Arena.
                                        //507 Win 20 battles in the Team Qualifier.
                ...
                case ActionID.ClaimPrize:
                    {
                        if (Pool.TableHeroRewards.ContainsKey(Prize / 100))
                        {
                            var DBInfo = Pool.TableHeroRewards[Prize / 100];
                            ...
                            if (DBInfo.ArrayItem.ContainsKey(Prize) && user.HeroRewards.ArrayGoals.TryGetValue(Prize, out prize))
                            {
                                if (prize.Claim == 0)
                                {
                                    ...
                                    if (user.Inventory.HaveSpace(count))
                                    {
                                        ...
                                        user.Inventory.Add(stream, ItemID, count, plus, 0, 0, Role.Flags.Gem.NoSocket, Role.Flags.Gem.NoSocket, true);
                ...
                case ActionID.ClaimSubPrize:
                    {
                        if (Pool.TableHeroRewards.ContainsKey(Prize))
                        {
                            var DBInfo = Pool.TableHeroRewards[Prize];
        ...
        public static void SendDone(Client.GameClient client)
        {
            //byte[] Packet = new byte[114];
            //VirusX.Game.MsgServer.Packet.WriteUInt16((ushort)(Packet.Length - 8), 0, Packet);
            //VirusX.Game.MsgServer.Packet.WriteUInt16((ushort)GamePackets.MsgProcessGoalSchedule, 2, Packet);
            //VirusX.Game.MsgServer.Packet.Ushort(10, 4, Packet);//count
            //int offset = 6;
            //uint uid = BitConverter.ReadUint(Packet, 10);
            //for (uint i = 0; i < 10; i++)
            //{
            //    VirusX.Game.MsgServer.Packet.Uint(i + 1, offset, Packet); offset += 5;//id
            //    if (MsgProcessGoal.Stages.ContainsKey(uid))
            //        Packet[offset++] = 1;//has something to claim
            //    else
            //        Packet[offset++] = 0;//has something to claim
            //    VirusX.Game.MsgServer.Packet.Uint(4, offset, Packet); offset += 4;//items done
            //}
            //client.Send(Packet);
        }
    }
