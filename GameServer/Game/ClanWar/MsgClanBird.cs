...
namespace VirusX.Game.MsgTournaments
{
    public class MsgClanBird
    {
        ...
        public static ushort MapID = 12025;
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Pool.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 15011));
        }

        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Hour == 21 && Now.Minute >= 49 && Now.Minute <= 58)
                this.Start();

            if (Proces == ProcesType.Alive)
            {
                if (Now.Hour == 21 && Now.Minute == 54 && Now.Second <= 2)

                if (Now.Hour == 21 && Now.Minute == 58 && Now.Second <= 2)

                if (Now.Hour == 21 && Now.Minute == 59 && Now.Second <= 2)
                    this.End();
            }
        }
        ...
        public void Start()
        {
            ...
            Furnitures[Role.SobNpc.StaticMesh.Pole].Name = "ClanBird";
        }
        ...
        {
            SortScores(true);
            MsgSchedules.SendSysMesage("The Round Ownered by Clan " + RoundOwner.ClanName + ".", MsgServer.MsgMessage.ChatMode.Center);
            Reset(stream);
        }
        public void End()
        {
            if (Proces == ProcesType.Alive)
            {
                ...
                if (RoundOwner != null)
                {
                    var ClanWinner = Pool.GamePoll.Values.Where(p => p.Player.MyClan != null && p.Player.MyClan.ID == RoundOwner.ClanID && p.Player.Map == MapID).ToArray();
                    if (ClanWinner != null)
                    {
