    ...
    public class MsgClassPKWar
    {
        ...
        internal void Start()
        {
            if (Proces == ProcesType.Dead)
            {
                ...
                foreach (var tournament in PkWars)
                {
                    ...
                    {
                        war.Start(Pool.ServerMaps[MapID]);
                    }
                }
        ...
        internal MsgServer.MsgUpdate.Flags ObtainAura(TournamentType typ)
        {
            switch (typ)
            {
                ...
                case TournamentType.Thunderstriker: return MsgServer.MsgUpdate.Flags.TopThunderstriker;
            }
        ...
        internal static TournamentType GetMyTournamentType(Client.GameClient client)
        {
            ...
            if (Database.AtributesStatus.IsThunderStriker(client.Player.Class))
                return TournamentType.Thunderstriker;
        ...
        public class War
        {
            ...
            public void Start(Role.GameMap map)
            {
                if (Proces == ProcesType.Dead)
                {
                    ...
                    {
                        client.Player.MessageBox("", new Action<Client.GameClient>(p => p.Teleport(312, 148, 1002, 0)), null, 60, MsgServer.MsgStaticMessage.Messages.ClassPk);
            ...
            public void GetMyReward(Client.GameClient client,ServerSockets.Packet stream)
            {
                if (IsFinish(client))
                {
                    ...
                    int Prize = 0;
                    if (Level == TournamentLevel.Level_130_140)
                    {
                        Prize = NewSystem.PrizeAllTops.ClassPkLevel_130_140;
                    }
                    if (Level == TournamentLevel.Level_120_130)
                    {
                        Prize = NewSystem.PrizeAllTops.ClassPkLevel_130_140;
                    }
                    if (Level == TournamentLevel.Level_100_119)
                    {
                        Prize = NewSystem.PrizeAllTops.ClassPkLevel_130_140;
                    }
                    if (Level == TournamentLevel.Level_1_99)
                    {
                        Prize = NewSystem.PrizeAllTops.ClassPkLevel_1_99;
                    }
                    client.Player.ConquerPoints += Prize;


                    client.SendSysMesage("You received " + Prize + " ConquerPoints. ", MsgServer.MsgMessage.ChatMode.System, MsgServer.MsgMessage.MsgColor.red);

                    LastFlag = aura;
                    Winner = client.Player.UID;

                    MsgSchedules.SendSysMesage("" + client.Player.Name + " Won " + Typ.ToString() + " PK War (" + Level.ToString() + ") , he received Top " + Typ.ToString() + ", " + NewSystem.PrizeAllTops.ClassPK.ToString() + " ConquerPoints!", MsgServer.MsgMessage.ChatMode.TopLeftSystem, MsgServer.MsgMessage.MsgColor.white);

                    client.Teleport(452, 295, 1002, 0);
                }
            }
        }
    }
