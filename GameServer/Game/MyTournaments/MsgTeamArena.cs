...
namespace ConquerOnline.Game.MsgTournaments
{
    public class MsgTeamArena
    {
        ...
        public List<uint> BlockArenaMaps = new List<uint>() { 4020,3820,5040,2057, 4000,6546,6525,8009, 6729,3825, 4003, 5342,4006, 4008, 4009 ,6000, 6001,6521,9988,6891,5599,8521, 1017,1080,1081,2060, 6002, 6003, 601, 700, 1038,MsgClassPKWar.MapID, 1036, 1764, Game.MsgTournaments.MsgEliteGroup.WaitingAreaID
        , 1357};
        ...
        public Extensions.Time32 CreateMatchesStamp = Extensions.Time32.Now.AddMilliseconds(ThreadFunctions.TeamArena_CreateMatches);
        public Extensions.Time32 VerifyMatchesStamp = Extensions.Time32.Now.AddMilliseconds(ThreadFunctions.TeamArena_VerifyMatches);
        public Extensions.Time32 CheckGroupsStamp = Extensions.Time32.Now.AddMilliseconds(ThreadFunctions.TeamArena_CheckGroups);
        ...
        public void CheckGroups(Extensions.Time32 clock)
        {
            if (clock > CheckGroupsStamp)
            {
                ...
                CheckGroupsStamp.Value = clock.Value + ThreadFunctions.TeamArena_CheckGroups;
            }
        }
        public void CreateMatches(Extensions.Time32 clock)
        {
            if (clock > CreateMatchesStamp)
            {
                ...
                CreateMatchesStamp.Value = clock.Value + ThreadFunctions.TeamArena_CreateMatches;
            }
        }
       public void VerifyMatches(Extensions.Time32 clock)
        {
            if (clock > VerifyMatchesStamp)
            {
                ...
                VerifyMatchesStamp.Value = clock.Value + ThreadFunctions.TeamArena_VerifyMatches;
            }
        }
        public unsafe class Match
        {
            ...
            public void CheckEnd(Client.GameClient client, bool disconnect = false)
            {
                bool allDead = true;
                if (client == null) return;
                if (client.Team == null) return;
                if (client.Team.Members == null) return;
                foreach (var teammate in client.Team.Members.Values)
                    if (teammate.client.InTeamQualifier())
                        allDead &= !teammate.client.Player.Alive;
                if (client.Team.Leader.Player.UID == client.Player.UID && disconnect)
                    End(client.Team);
                else if (allDead)
                    if (client.Team.Leader.Player.UID == client.Player.UID)
                        End(client.Team);
                    else
                        foreach (var teammate in client.Team.Members.Values)
                            if (teammate.client.Team.Leader.Player.UID == teammate.client.Player.UID)
                                End(teammate.client.Team);
            }
            public void Win(Role.Instance.Team winner, Role.Instance.Team loser)
            {
                ...
                if (winner.TeamArenaMatch != null && loser.TeamArenaMatch != null)
                {
                    using (var rec = new ServerSockets.RecycledPacket())
                    {
                        ...
                        int diff = Pool.GetRandom.Next(30, 50);
 ...
 StringBuilder builder = new StringBuilder();
        ...
        {
            // client.SendSysMesage("Sorry, arena team is test now.", MsgServer.MsgMessage.ChatMode.Agate, MsgServer.MsgMessage.MsgColor.red);
            //  return;
