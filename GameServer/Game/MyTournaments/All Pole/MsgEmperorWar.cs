...
namespace VirusX.Game.MsgTournaments
{
    public class MsgEmperorWar
    {
        ...
        public static ushort MapID = 6525;
        ...
        public void Load()
        {
            Furnitures.Add(Role.SobNpc.StaticMesh.Pole, Pool.ServerMaps[MapID].View.GetMapObject<Role.SobNpc>(Role.MapObjectType.SobNpc, 55839));
        }

        public void CheckUP()
        {
            DateTime Now = DateTime.Now;
            if (Proces == ProcesType.Dead && Now.Minute >= 30 && Now.Minute <= 39)//44 mt8yrhash
                this.Start();

            if (Proces == ProcesType.Alive)
            {
                //if (Now.Minute == 10 && Now.Second <= 2)
                //    MsgSchedules.SendSysMesage("EmperorWar will ended after 20 Minutes.", MsgServer.MsgMessage.ChatMode.Center);
                if (Now.Minute == 37 && Now.Second <= 2)
                    MsgSchedules.SendSysMesage("EmperorWar will ended after 3 Minutes.", MsgServer.MsgMessage.ChatMode.Center);

                if (Now.Minute == 39 && Now.Second <= 2)
                    MsgSchedules.SendSysMesage("EmperorWar will ended after 1 Minutes.", MsgServer.MsgMessage.ChatMode.Center);
        ...
        public void Start()
        {
            ...
            MsgSchedules.SendSysMesage("EmperorWar war has began", MsgServer.MsgMessage.ChatMode.Center);
            MsgSchedules.SendSysMesage("EmperorWar war has began", MsgServer.MsgMessage.ChatMode.BroadcastMessage);
            MsgSchedules.SendInvitation("EmperorWar has begun! Would you like to join?", "CPs", 345, 327, 1002, 0, 60);

            Furnitures[Role.SobNpc.StaticMesh.Pole].Name = "EmperorWar";
        }
        ...
        public void End()
        {
            ...
            {
                Proces = ProcesType.Dead;
                MsgSchedules.SendSysMesage(RoundOwner == null ? "EmperorWar has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner of EmperorWar.", MsgServer.MsgMessage.ChatMode.Center);
                MsgSchedules.SendSysMesage(RoundOwner == null ? "EmperorWar has ended and there is no winner" : "Guild " + RoundOwner.GuildName + ", He is the winner of EmperorWar.", MsgServer.MsgMessage.ChatMode.BroadcastMessage);
                ScoreList.Clear();
            }
        }
